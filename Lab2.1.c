...
int main()
{
	...
	{
		while (!x1) {
			printf("Выберите что, вы хотите сделать: 1-Ввести сумму в белорусских рублях, 2-Вывод суммы в валюте, 
			       3-Таблица курсов валют, 4-Прибыль от операции для каждой валюты, 5-Краткая информация о банке, 
			       6-Выход, 7-Очистить консоль\n!ВНИМАНИЕ! Если вы введете число с количеством символов больше 1, 
			       за введенное число будет принята первая цифра\n");
			fflush(stdin);
			fgets(num, 2, stdin);
			if (!checkI(num))
...
void info()
{
	...
	printf("В 2014 году Приорбанк был удостоен награды «Самый быстрорастущий банк в сегменте «Средние клиенты» среди сетевых 
	       банков группы Райффайзен».\n");
